...
class FactoryGenerator
{
    ...
    {
        $shouldBeIncluded = (!$column['nullable'])
            && !$column['increments']
            && !$column['foreign']
     ...
     */
    protected function writeBaseFactoryFile(string $path, array $data, string $modelClass): void
    {
        $base_factory_dir = dirname($path);

        File::ensureDirectoryExists($base_factory_dir);
        ...
        $definitions = array_map(function ($line) use ($modelName)
        {
            ...
            // Match the column name inside the quotes before =>
            if (preg_match("/'([^']+)'/", $line, $matches))
            {
                $column = $matches[1];                  // e.g. name
                $constant = strtoupper($column);        // NAME
                // Replace "'column'" with "Model::COLUMN"
                $line = preg_replace(
                    "/'{$column}'/",
                    "{$modelName}::{$constant}",
                    $line
