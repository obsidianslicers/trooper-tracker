...
/**
 * Tests for the UpdateCostumesRequest class.
 *
 * Validates authorization logic and rule generation for costume updates.
 */
class UpdateCostumesRequestTest extends TestCase
{
    use RefreshDatabase;

    private UpdateCostumesRequest $subject;
    ...
    {
        parent::setUp();
        $this->subject = new UpdateCostumesRequest();
    ...
    public function test_rules_returns_validation_rules_for_costume_names(): void
    {
        // Act
        $rules = $this->subject->rules();

        // Assert
        $this->assertArrayHasKey('costumes.*.name', $rules);
        $this->assertIsArray($rules['costumes.*.name']);
        $this->assertContains('required', $rules['costumes.*.name']);
        $this->assertContains('string', $rules['costumes.*.name']);
        $this->assertContains('max:128', $rules['costumes.*.name']);

        // Check that UniqueCostumeNameRule is present
        $has_unique_rule = false;
        foreach ($rules['costumes.*.name'] as $rule)
        {
